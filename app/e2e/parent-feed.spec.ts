...
test('parent feed page has daily playlist and weekly deep dive', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: 'Malayalam', exact: true })).toBeVisible()
})
...
test('erotic shelf is 18+ gated and shows 150 official titles', async ({ page }) => {
  ...
  await page.getByTestId('erotic-open').first().click()
  await expect(page.getByTestId('movie-summary')).toBeVisible()
  await expect(page.getByTestId('movie-detail')).not.toContainText(/Amazon Prime Video|Netflix|MUBI|JustWatch|Score /i)
  await page.getByTestId('movie-back').click()
  await page.getByRole('button', { name: 'Bengali', exact: true }).click()
